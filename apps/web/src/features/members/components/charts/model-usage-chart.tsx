...
function groupModelUsage(models: ModelUsageBreakdown[]): ModelChartItem[] {
  ...
  if (models.length > MODEL_VISIBLE_LIMIT) {
    ...
    visible.splice(MODEL_VISIBLE_LIMIT - 1, 1, {
      ...
      cache_read_tokens: overflow.reduce((sum, item) => sum + item.cache_read_tokens, 0),
      cache_write_tokens: overflow.reduce((sum, item) => sum + item.cache_write_tokens, 0),
      estimated_cost_usd_micros: overflow.reduce((sum, item) => sum + item.estimated_cost_usd_micros, 0),
      cache_savings_usd_micros: overflow.reduce((sum, item) => sum + item.cache_savings_usd_micros, 0),
