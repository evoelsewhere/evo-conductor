...
export function SettingsPage() {
  ...
  const saveSso = useMutation({
    ...
    onError: (e) => setFormError(e instanceof Error ? e.message : "SSO save failed"),
  })
  ...
  return (
    <PageFrame
      ...
      subtitle="Update project identity and SSO after initial setup."
    >
      {message && (
        <div className="mb-4 rounded-lg border border-(--color-success)/30 bg-(--color-success)/10 px-3 py-2 text-sm text-(--color-success)">
          {message}
        </div>
      )}
      {formError && <ErrorState className="mb-4" message={formError} />}
